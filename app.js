(() => {
  ...
  const revealPreloader = () => {
    if (!preloader || preloaderRevealed) return;
    preloaderRevealed = true;
    preloader.classList.add("is-hidden");
  };
  requestAnimationFrame(() => setTimeout(revealPreloader, 180));
  window.addEventListener("load", () => setTimeout(revealPreloader, 90), { once: true });
  setTimeout(revealPreloader, 1500);
  ...
  const introVisual = $("[data-intro-visual]");
  if (introVisual && !reducedMotion && matchMedia("(hover: hover) and (pointer: fine)").matches) {
    let introPointerRaf = 0;
    let introPointerEvent = null;

    const renderIntroPointer = () => {
      const rect = introVisual.getBoundingClientRect();
      const x = clamp((introPointerEvent.clientX - rect.left) / rect.width);
      const y = clamp((introPointerEvent.clientY - rect.top) / rect.height);
      introVisual.style.setProperty("--intro-x", `${(x * 100).toFixed(2)}%`);
      introVisual.style.setProperty("--intro-y", `${(y * 100).toFixed(2)}%`);
      introVisual.style.setProperty("--intro-shift-x", `${((x - .5) * -8).toFixed(2)}px`);
      introVisual.style.setProperty("--intro-shift-y", `${((y - .5) * -5).toFixed(2)}px`);
      introPointerRaf = 0;
    };

    introVisual.addEventListener("pointermove", event => {
      introPointerEvent = event;
      if (!introPointerRaf) introPointerRaf = requestAnimationFrame(renderIntroPointer);
    }, { passive: true });

    introVisual.addEventListener("pointerleave", () => {
      introVisual.style.setProperty("--intro-x", "50%");
      introVisual.style.setProperty("--intro-y", "45%");
      introVisual.style.setProperty("--intro-shift-x", "0px");
      introVisual.style.setProperty("--intro-shift-y", "0px");
    }, { passive: true });
  }

  const sitePointerLight = $(".site-pointer-light");
  if (sitePointerLight && !reducedMotion && matchMedia("(hover: hover) and (pointer: fine)").matches) {
    let pointerRaf = 0;
    let pointerEvent = null;

    const renderPointerLight = () => {
      if (!pointerEvent) return;
      sitePointerLight.style.setProperty("--site-pointer-x", `${pointerEvent.clientX}px`);
      sitePointerLight.style.setProperty("--site-pointer-y", `${pointerEvent.clientY}px`);
      sitePointerLight.classList.add("is-active");
      pointerRaf = 0;
    };
    ...
    document.addEventListener("pointermove", event => {
      ...
      pointerEvent = event;
      if (!pointerRaf) pointerRaf = requestAnimationFrame(renderPointerLight);
    }, { passive: true });

    document.addEventListener("pointerleave", () => {
      sitePointerLight.classList.remove("is-active");
    }, { passive: true });
    window.addEventListener("blur", () => sitePointerLight.classList.remove("is-active"), { passive: true });
  }
  ...
  const hydrateVideo = (video, options = {}) => {
  ...
  const scheduleVideoHydration = (video, delay, options = {}) => {
    const run = () => {
      if (document.visibilityState === "hidden") return;
      hydrateVideo(video, options);
    };
    if (options.eager) {
      const schedule = () => requestAnimationFrame(() => setTimeout(run, delay));
    ...
    window.addEventListener("load", () => {
      if ("requestIdleCallback" in window) {
        requestIdleCallback(run, { timeout: delay + 500 });
      } else {
  ...
  if (heroVideo) {
    ...
    const heroHydrationDelay = constrainedConnection ? 900 : (isMobileViewport ? 220 : 90);
    const heroPreload = constrainedConnection ? "metadata" : "auto";
    const playHeroVideo = () => {
      if (!reducedMotion && !heroStage.classList.contains("is-scrubbing")) {
    ...
    scheduleVideoHydration(heroVideo, heroHydrationDelay, {
      eager: true,
      preload: heroPreload
    });
    heroStage.addEventListener("pointerenter", () => hydrateVideo(heroVideo, { preload: heroPreload }), { passive: true });
  }
  ...
  const renderScroll = () => {
    ...
    if (!reducedMotion) {
      ...
      heroCopy.style.transform = mobileViewport
    ...
    journeyScenes.forEach((scene, index) => {
      ...
      if (!reducedMotion) {
        scene.style.transform = mobileViewport
          ? `translate3d(0, ${delta * 48}vh, 0)`
          : `translate3d(0, ${delta * 68}vh, ${-distance * 520}px) rotateX(${delta * -5}deg)`;
  ...
  if (!heroMotionEnabled) {
    ...
    const renderVideoScrub = () => {
      ...
      if (heroVideo.duration) {
        scrubCurrent += (scrubTarget - scrubCurrent) * .1;
        const targetTime = Math.min(heroVideo.duration - .04, Math.max(.04, scrubCurrent * heroVideo.duration));
        const now = performance.now();
        if (!heroVideo.seeking && now - lastVideoSeek > 140 && Math.abs(heroVideo.currentTime - targetTime) > .12) {
  ...
  const galleryData = [
    ...Array.from({ length: 25 }, (_, index) => ({
      category: "gym",
      src: `assets/images/gym-${String(index + 1).padStart(2, "0")}.webp`,
      alt: `فضای باشگاه MG، تصویر ${index + 1}`
    })),
    ...Array.from({ length: 4 }, (_, index) => ({
      category: "training",
      src: `assets/images/training-${String(index + 1).padStart(2, "0")}.webp`,
      alt: `تمرین تخصصی بانوان، تصویر ${index + 1}`
    })),
    ...Array.from({ length: 6 }, (_, index) => ({
      category: "buffet",
      src: `assets/images/buffet-${String(index + 1).padStart(2, "0")}.webp`,
      alt: `بوفه سلامت MG، تصویر ${index + 1}`
    })),
    ...Array.from({ length: 6 }, (_, index) => ({
      category: "manager",
      src: `assets/images/manager-${String(index + 1).padStart(2, "0")}.webp`,
  ...
  if (motionVideo && motionControl) {
    ...
    motionVideo.addEventListener("loadeddata", () => {
      if (motionInView && !motionManualPause && !reducedMotion) {
    ...
    const motionObserver = new IntersectionObserver(entries => {
      entries.forEach(entry => {
        ...
        if (reducedMotion || !motionInView || motionManualPause || motionScrubbing) {
          ...
          hydrateVideo(motionVideo);
          if (motionVideo.dataset.loaded === "true") {
            motionVideo.play().catch(() => {});
          }
        }
        updateVideoControl();
